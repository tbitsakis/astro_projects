...
'''
Description /
This script reads the outputs of daoproc_v2 and creates 
         catalogues of the photometry, performs statistics in the 
         outputs, and creates the ds9 region files. 

inputs: *.als files from daoproc_v2.
...
- v3 17/10/2017: version for FORS camera (Stavros project), eg. removing 
    ZP+color terms
- v2 23/04/2015: adding output for selected chi2 sources, and ZP+color 
    term as obtained through the manual for the CTIO data, currently the .mag 
    option is not working since the "system parameters" are commented out 
- v1 30/06/2014: info output with more details and organized for later use

'''

# TODO insert mag sharpness, chi2 selection and limit if wanted...else prin the whole sample!
...
"""
# SYSTEM PARAMETERS -- change accordingly (depending on system/coaddittion method) / used for .mag inputs to calculate snr
igain = 2.2     # individual gain in e/ADU
Nimages = 6.    # number of images
gain = igain*Nim ages     # effective gain
texp = 12    # in s
nd = 0.00105     # dark current in e/px/s        ??? WHAT is the VALUE for this (for CTIO camera)
readnoise = 5     # read noise in e/px (/read)
nr = sqrt(Nimages)*readnoise # effective readnoise  

selmagerr = 0.05    # selecting sources with mag error smallen than 
"""
...
for infile in arg:
    ...
    g.write('global color=cyan dashlist=8 3 width=1 font="helvetica 10 normal roman" select=1 highlite=1 dash=0 fixed=0 edit=1 move=0 delete=1 include=1 source=1\nimage\n')
    d.write('#star_id  xpos  ypos  mag  magerr  sharpness  chi2\n')


    sef = open(infile+'.selected.reg','w')  # selected sources
    sef.write('global color=magenta dashlist=8 3 width=1 font="helvetica 10 normal roman" select=1 highlite=1 dash=0 fixed=0 edit=1 move=0 delete=1 include=1 source=1\nimage\n')
...
#           mag = float(line0[3])+0.631+0.032*1.536
            ...
            if chi>=0.5 and chi<=2.5 and sharp>-0.5 and sharp<0.5:
#            if mag>=18.1 and mag<=18.8:
#                print star_id, chi, sharp
                sef.write('point('+str(xpos)+','+str(ypos)+') # point=cross\n')
...
#            if float(sharp)>-0.5 and sharp<0.5 and float(chi)<1.5: # and magerr<1:
            if magerr<99:
                ...
                achi.append(chi)
                anitr.append(nitr)        
                g.write('point('+str(xpos)+','+str(ypos)+') # point=cross\n')
            else:
